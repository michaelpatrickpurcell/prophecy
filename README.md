...
See the [rules pages](prophecy_pages.pdf) for a typeset version of the rules designed to be read on screens.

See the [rules booklet](prophecy_booklet.pdf) for a version of the rules that can be printed to create an A5-size booklet.
...
To play the game the players will need a few materials that they will use to take notes and determine outcomes. They will need:
  ...
  - __Butcher Paper__: One large sheet of butcher paper should be placed in the middle of the play area.  A whiteboard can be used instead if desired. This is the _story board_.
  - __Markers__: Each player should have a marker to write on the index cards, sticky notes, and story board.
...
An aspect that describes an object is _attached_ to that object.
...
#### Characters
A _character_ is a person who appears in the story and who  is controlled by a single player throughout the game.
...
#### Environment
Any object that is not a character is a part of the _environment_.
...
An aspect that is attached to any part of the environment is an _environment aspect_.
...
#### Example
Suppose that the players are telling a story in which the characters are trying to deal with the prophecy described in [The Tachyonic Antitelephone](#the-tachyonic-antitelephone).
...
Several aspects are attached to Ruth, including:
  - DARPA research scientist
  - Independently wealthy
  - Acerbic wit
  - Brilliant physicist

Several aspects are attached to the guard, including:
  - Overworked
  - Underpaid

The players identify a candidate pair of aspects:
  - Independently wealthy / Underpaid.
...
### Checks
The players make a _check_ to determine the outcome of each scene.
To make a check, the players will:
  - __Assemble a Dice Pool__:
   ...
   - __Roll the Dice__:
     ...
     That is, for every die that yields `6` another die is added to the pool.
  - __Compute the Result__:
     Any die that yields `1`, `2`, or `3` is a _miss_.
     Any die that yields `4`, `5`, or `6` is a _hit_.
     The _result_ of a roll is the total number of hits.
  - __Determine the Outcome__:
...
#### Example
Suppose that the players are making a check to determine the outcome of a scene where the characters are trying to deal with the prophecy described in [Project Spaceguard](#project-spaceguard).

The objective of the scene is:
  - Smuggle the children aboard a colony ship.
...
The players identify three pairs of matching aspects that the characters can use to manipulate the scene to their advantage:
  - Defence contractor / Built by the lowest bidder,
  - Armed and dangerous / I'm a doctor not a soldier,
  - Expert hacker / It's a UNIX system!

Two of the scene's precursors were resolved successfully. The objectives of those scenes were:
  - Befriend one of the flight controllers,
  - Steal a set of crew uniforms.

So, the dice pool initially consists of five dice.

When rolled, these dice yield `3`, `6`, `5`, `1`, `6`.
Because two dice yielded `6`, two dice are added to the pool.
When rolled, these dice yield `2`, `6`.
Because one die yielded `6`, another die is added to the pool.
When rolled, this die yields `4`.

Altogether, the final roll yields `3`, `6`, `5`, `1`, `6`, `2`, `6`, `4`.

Because five dice yielded `4`, `5`, or `6`, the result of this roll is five hits.
...
The players will:
  - [Receive a Prophecy](#receive-a-prophecy): The prophecy describes an impending catastrophe for some fictional world.
  - [Create Characters](#create-characters): The characters inhabit the fictional world that the players create.
  - [Write an Outline](#write-an-outline): The outline describes the story that the players will tell about the characters.
...
The players should first sketch a scene called the _finale_.
...
## The Moderator
Optionally, one of the players can assume the role of _moderator_. The moderator's job is to help the other players play the game. This assistance can take a variety of forms: answering questions about the rules, managing logistics, or ensuring that everyone agrees about various details of the story.
...
## Modules
The first thing that the players do is [Receive a Prophecy](#receive-a-prophecy).
...
I am, well ... I'm you.  That is, I was. Or, you will be.  Or, something like that. Anyway, I'm afraid that I've got some bad news. Really bad. You see, it turns out that the work that you did for the government last year was used to create something that we used to think impossible.
...
### Playtesters
The following people helped to refine the design of Prophecy:
  - Keydan Bruce
  - Farzana Choudhury
  - Michael Cromer
  - Dannielle Harden
...
The following games influenced the design of Prophecy:
  - __Fate__: Aspects and Objects.
  - __Fiasco__: Scenes and scene-level resolution.
  - __Our Last Best Hope__: Theme and narrative focus.
  - __10 Candles__: Session modules.
  - __Microscope__: Non-linear storytelling.

[Safety Tools](#safety-tools) is based on the X-Card by John Stavropoulus.

### Design Tools
The following tools were used to create this booklet:
  - __XeLaTeX__: Typesetting and layout.
  - __TikZ__: Diagrams and art.
