...
References to sections of the rules will consist of a [hyperlink](#prophecy) to the target section.
...
To make a Check, the players will:
  ...
  2. __Roll the Dice__ -
     ...
     That is, for every die that yields `6` one additional die is added to the dice pool.
...
Because one of the dice yielded {`6`}, one additional die is added to the dice pool.
...
#### Example
This diagram depicts one possible outline for a story about how characters might try to deal with the prophecy described in [Return of the Dragon](#return-of-the-dragon):
```
...
|- Discover the Dragon's Weakness (3)
   |- Find Lost Stories About the Dragon (2)
```
...
By keeping the game moving, the moderator can help to ensure that the story stays tightly focused on the characters and their attempts to deal with the catastrophe.
