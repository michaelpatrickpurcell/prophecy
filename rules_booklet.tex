...
\usepackage[type={CC}, version={4.0}, modifier={by}]{doclicense}
...
\begin{titlepage}
		...
		\raggedright{\Large{{\textcolor{black}{Michael Purcell \hfill Version 0.2}}}}
...
No player should ever try to unilaterally dictate what happens in the story and no player should ever feel like their contributions have ignored or overruled by another player.
...
If an aspect is attached to an object, then the players should stick the note for that aspect to the index card for that object.
...
Note that, within the framework of the game, most of the people who appear in the game are not characters, they are part of the environment. 
An aspect that is attached to any part of the environment is an \emph{environment~aspect}.

\newpage

\subsection*{Matching Aspects}
...
For each candidate pair, the players should discuss how the interaction between the two aspects would make it easier for the characters to accomplish their objective in the scene. If If the players agree that the pair would do so, then they are matching aspects. Otherwise, they are not.

\subsubsection*{Example}
Suppose that the players are telling a story in which the characters are trying to deal with the prophecy described in {\cinzel \small The Tachyonic Antitelephone}. 
...
\begin{multicols}{2}
\begin{itemize}[noitemsep,nolistsep,  leftmargin=0.7cm]
...
\subsubsection*{Example}
Suppose that the players are making a check to determine the outcome of a scene where the characters are trying to deal with the prophecy described in {\cinzel \small Project Spaceguard}.
...
The characters are able to sneak the children aboard a colony ship.
...
\subsubsection*{Sketch the Finale}
...
\subsubsection*{Sketch Additional Scenes}
...
\subsubsection*{Perform the Finale}
...
\newpage
\subsection*{Unsafe at Any Speed \setmainfont{URWClassico} - By Paul Murray}
...
\subsection*{The Deluge \setmainfont{URWClassico}- From The King James Version of the Bible}
...
\subsection*{The Tachyonic Antitelephone}
...
\begin{multicols}{2}
\begin{itemize}[noitemsep,nolistsep,  leftmargin=0.7cm,topsep=-1ex]%\widthof{\hspace{0.4cm}}]
  \item Keydan Bruce
...
\begin{description}[labelindent=0.25cm, leftmargin=\widthof{\hspace{0.25cm}\textbullet\space}, font=\normalfont\textbullet\space, noitemsep, topsep=-1ex]%[leftmargin=\widthof{\hspace{0.25cm}\textbullet\space}, noitemsep, topsep=-1ex]
...
\end{description}
\vspace{1ex}
{\cinzel \small Safety Tools} is based on the X-Card by John Stavropoulus.
...
{\cinzel\bfseries\small{Contact}} - prophecy.ttrpg@gmail.com\\
